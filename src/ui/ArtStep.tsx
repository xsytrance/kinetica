...
import { startConnect, disconnect as disconnectOpenRouter } from "@/ai/openrouterAuth";
...
export function ArtStep({ track, duration, onDone }: {
  ...
  const orConnected = !!orKey.trim();
  // Pick up the key when "Connect with OpenRouter" completes (popup or redirect).
  useEffect(() => {
    const on = () => setOrKey(localStorage.getItem("kinetica-openrouter-key") ?? "");
    window.addEventListener("kinetica-or-connected", on);
    return () => window.removeEventListener("kinetica-or-connected", on);
  }, []);
    ...
    <div className="mx-auto flex min-h-[100dvh] max-w-2xl flex-col justify-center gap-5 p-6">
      ...
      <div className="rounded-xl border border-white/10 p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {(([["off", "Off (free)"], ["openrouter", "OpenRouter"], ["local", "Local · Ollama+ComfyUI"]]) as [Engine, string][]).map(([id, label]) => (
